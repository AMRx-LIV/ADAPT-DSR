...
prev_ICD_applier <- function(df,icd_df,prefix,codes) {
  ...
  urine_dsr <- reduce(codes, function(df, code) {
...
setwd("/Users/alexhoward/Documents/Projects/ADAPT-DSR/ADAPT-DSR code")
...
urine_dsr <- urine_dsr %>% MIMER::check_previous_events(cols="complicated_vs_uncomplicated_uti", sort_by_col='charttime',
                             patient_id_col='subject_id', event_indi_value='1',
                             new_col_prefix="pr_",
                             time_period_in_days = 1e4, minimum_prev_events = 1,
                             default_na_date = '9999-12-31 00:00:00') %>% ungroup()
...
urine_dsr <- reduce(seq_along(antibiotics), function(df, i) {
...
urine_dsr <- urine_dsr %>% care_event_assigner(uti_df,"(urin|pyelo|cath)",long_title,pUTI,"admittime",365,1)

###Presence of an outpatient provider ID
urine_dsr <- urine_dsr %>% mutate(provider_id = case_when(order_provider_id!="" ~ TRUE,
                                                            TRUE ~ FALSE))

###Current inpatient specialty
serv_key <- services %>% select(hadm_id,curr_service) %>% distinct(hadm_id,.keep_all = T)
urine_dsr <- urine_dsr %>% left_join(serv_key,by="hadm_id") %>% mutate(
...
urine_dsr <- urine_dsr %>% mutate(ordertime=chartdate) %>%
...
prev_ICD_applier <- function(df,icd_df,prefix,codes) {
  ...
  blood_dsr <- reduce(codes, function(df, code) {
    apply_prev_event_assignments(df, code)
  }, .init = blood_dsr) %>%
    mutate(pDIAG_U = FALSE) %>%
    ungroup()
...
###Finding previous complicated uti
blood_dsr <- blood_dsr %>% MIMER::check_previous_events(cols="uti_vs_not_uti", sort_by_col='charttime',
                                                        patient_id_col='subject_id', event_indi_value='1',
                                                        new_col_prefix="pr_",
                                                        time_period_in_days = 1e4, minimum_prev_events = 1,
                                                        default_na_date = '9999-12-31 00:00:00') %>% ungroup()
...
blood_dsr <- reduce(seq_along(antibiotics), function(df, i) {
  apply_prev_rx(df, suffixes[i], antibiotics[i])
}, .init = blood_dsr) %>%
  ungroup()
...
uti_df <- diagnoses_raw %>% left_join(uti_key,by=c("icd_code","icd_version")) %>% 
  filter(!is.na(long_title)) %>% left_join(hadm_key,by="hadm_id")
blood_dsr <- blood_dsr %>% care_event_assigner(uti_df,"(urin|pyelo|cath)",long_title,pUTI,"admittime",365,1)

###Presence of an outpatient provider ID
blood_dsr <- blood_dsr %>% mutate(provider_id = case_when(order_provider_id!="" ~ TRUE,
                                                          TRUE ~ FALSE))

###Current inpatient specialty
serv_key <- services %>% select(hadm_id,curr_service) %>% distinct(hadm_id,.keep_all = T)
blood_dsr <- blood_dsr %>% left_join(serv_key,by="hadm_id") %>% mutate(
  curr_service = case_when(is.na(curr_service) ~ "UNKNOWN",
                           TRUE ~ curr_service))
colnames(blood_dsr)
###At least one measured obese, underweight, or overweight BMI category in the last 3 years
...
obs <- poe %>% filter(order_subtype=="Vitals/Monitoring") %>% 
  ...
  group_by(subject_id,ordertime) %>% count(order_subtype) %>% 
  arrange(desc(n)) %>% select(-order_subtype)
blood_dsr <- blood_dsr %>% mutate(ordertime=chartdate) %>%
  left_join(obs,by=c("subject_id","ordertime")) %>% 
  rename(ob_freq = "n") %>% mutate(ob_freq = case_when(is.na(ob_freq) ~ 0,
                                                       TRUE ~ ob_freq),
                                   ob_freq = standardize(ob_freq)) %>% 
...
###Diabetes
diabkey <- drgcodes %>% filter(grepl(
  "(DIAB)",
  description)) %>% 
  select(hadm_id,description) %>% distinct(hadm_id)
hadm <- hadm %>% left_join(diabkey,by="hadm_id")
urine_dsr <- urine_dsr %>% mutate(charttime=admittime) %>% 
...
write_csv(urine_dsr,"preprocessed_urines2.csv")
write_csv(blood_dsr,"preprocessed_blood2.csv")
